...
public class Teleop extends LinearOpMode {
    ...
    private void Shooter() {
        if (gamepad2.left_trigger > 0) {
            ...
            if ((shooter.getVelocity() > targetSpeed - shootingTolerance) && (shooter.getVelocity() < targetSpeed + shootingTolerance)) {
                sleep(100);
                if ((shooter.getVelocity() > targetSpeed - shootingTolerance) && (shooter.getVelocity() < targetSpeed + shootingTolerance)) {
        ...
        if (currentDetections.size() == 1) {
            for (AprilTagDetection detection : currentDetections) {
                if (detection.ftcPose != null) {
                    targetSpeed = 6.7 * detection.ftcPose.y + 922;
                    shootingTolerance = 130 - detection.ftcPose.y;
                }
            }
        } 
    }
            ...
            if (currentDetections.size() == 1) {
                for (AprilTagDetection detection : currentDetections) {
                    if (detection.id == 20 && detection.ftcPose != null) {
                        ...
                        if (detection.ftcPose.z > -0) {
                            Back_left.setPower(0.1);
                            Front_left.setPower(0.1);
                            Back_right.setPower(-0.1);
                            Front_right.setPower(-0.1);
                        } else if (detection.ftcPose.z < -3) {
                            Back_left.setPower(-0.1);
                            Front_left.setPower(-0.1);
                            Back_right.setPower(0.1);
                            Front_right.setPower(0.1);
